...
namespace socketDll
{
    ...
    {
        private bool IsStop = false;    //关闭服务器
        readonly object obj = new object();
        public static PushSockets pushSockets;  //推送事件

        /// <summary>  
        /// 信号量  初始化5个信号量，最高并发500
        /// </summary>  
        private Semaphore semap = new Semaphore(5, 500);
        ...
        private void GetAcceptTcpClient()
        {
            try
            {
                if (Listener.Pending()) //listen等待队列是否不为空
                {
                    semap.WaitOne();    //阻塞
                    //接收到挂起的客户端请求链接  
                    ...
                    Sockets sks = new Sockets(tcpClient.Client.RemoteEndPoint 
                        as IPEndPoint, tcpClient, stream);  //增加sks客户端
                    ...
                    //客户端异步接收数据  
                    sks.nStream.BeginRead(sks.RecBuffer, 0, sks.RecBuffer.Length, 
                        new AsyncCallback(EndReader), sks);
        ...
        {
            Sockets sks = ir.AsyncState as Sockets; //异步操作的信息转换为sks引用
            if (sks != null && Listener != null)
            {
                try
                {
                    if (sks.NewClientFlag || sks.Offset != 0)   //新客户端或有接收消息
                    {       
                        sks.NewClientFlag = false;
                        sks.Offset = sks.nStream.EndRead(ir);
                        //推送到netstat  ⭐⭐ ⭐⭐
                        pushSockets.Invoke(sks);
                        sks.nStream.BeginRead(sks.RecBuffer, 0, 
                            sks.RecBuffer.Length, new AsyncCallback(EndReader), sks);
                    }
                }
